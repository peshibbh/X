...
export const makeChatsSocket = (config) => {
    const { logger, markOnlineOnConnect, fireInitQueries, appStateMacVerification, shouldIgnoreJid, shouldSyncHistoryMessage, processHistorySyncOnBoot, getMessage } = config;
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        // Unless processHistorySyncOnBoot is enabled, only the initial/latest sync is
        // processed — reconnects skip the full/boot history re-hydration and the full
        // app-state resync that would otherwise run on every reconnection.
        const isLatestHistorySync = !authState.creds.processedHistoryMessages?.length;
        const shouldProcessHistoryMsg = historyMsg
            ? (processHistorySyncOnBoot || isLatestHistorySync) &&
